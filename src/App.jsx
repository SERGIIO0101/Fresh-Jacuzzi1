...
export default function App() {
  ...
  // 1. Efecto para el scroll y el preloader
  useEffect(() => {
    const handleScroll = () => {
  ...
  // 2. Efecto para resetear el scroll al cambiar de página
  useEffect(() => {
    window.scrollTo(0, 0);
  }, [pathname]);
  ...
  return (
    <div className="min-h-screen flex flex-col bg-white antialiased selection:bg-fresh-gold/20 selection:text-fresh-navy overflow-x-hidden relative">
      ...
      {/* 3. ESPACIADOR DINÁMICO: 
          Previene que el contenido se oculte tras el header fijo. 
          Ajusta estos valores según la altura real de tus componentes. */}
      ...
      >
        <span className="hidden md:block mr-4 bg-fresh-navy text-white text-[10px] font-black uppercase tracking-widest py-3 px-6 rounded-full shadow-2xl opacity-0 translate-x-4 group-hover:opacity-100 group-hover:translate-x-0 transition-all duration-500 border border-white/10">
          Consultoría Técnica
        </span>
        <div className="w-14 h-14 md:w-16 md:h-16 bg-[#25D366] text-white rounded-full shadow-2xl flex items-center justify-center hover:scale-110 transition-all duration-500">
